                    ...
                    <form method="POST" action="{{ route('admin.periodos.update', $periodo) }}">
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            <div>
                                ...
                                <x-text-input id="anio" name="anio" type="number" class="mt-1 block w-full" :value="old('anio', $periodo->anio)" min="2020" max="2050" required />
                            ...
                            <div>
                                ...
                                <select id="semestre" name="semestre" class="mt-1 block w-full border-gray-300 focus:border-primary-500 focus:ring-primary-500 rounded-md shadow-sm" required>
                                    <option value="I" {{ old('semestre', $periodo->semestre) == 'I' ? 'selected' : '' }}>I</option>
                                    <option value="II" {{ old('semestre', $periodo->semestre) == 'II' ? 'selected' : '' }}>II</option>
                                </select>
                            ...
                            <div class="col-span-2">
                                <label class="inline-flex items-center">
                                    <input type="checkbox" name="activo" value="1" class="rounded border-gray-300 text-primary-600 shadow-sm focus:ring-primary-500" {{ old('activo', $periodo->activo) ? 'checked' : '' }}>
                                    <span class="ml-2 text-sm text-gray-600">Período activo</span>
                                </label>
                            </div>
                        </div>
